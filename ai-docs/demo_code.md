...
def fix_and_parse_json(response_text):
    ...
    try:
        ...
        corrected_text = re.sub(r'([{,])\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*:', r'\1"\2":', response_text)
...
def process_single_table(metadata):
    ...
    if not all([org_id, tbl_id, tbl_nm]):
        print(f"  [건너뛰기] 필수 메타데이터(ORG_ID, TBL_ID)가 없어 {tbl_nm} 처리를 건너뜁니다.")
        return
    ...
    for prd_se, prd_name in period_priority:
        ...
        try:
            ...
            if data and "errMsg" not in data:
                ...
                print(f"  ✔ 성공: 주기 '{selected_period_name}'에서 데이터 {len(raw_data)}건을 찾았습니다.")
                break # 데이터 찾으면 루프 종료
            else:
    ...
    if not raw_data:
        print(f"  ❌ 실패: {tbl_nm}({tbl_id})에 대한 데이터를 어떤 주기에서도 가져올 수 없습니다.")
        return
...
def main():
    ...
    if not INPUT_METADATA_FILE.exists():
        print(f"❌ 오류: 메타데이터 파일({INPUT_METADATA_FILE})이 없습니다. 이전 스크립트를 먼저 실행해주세요.")
        return
