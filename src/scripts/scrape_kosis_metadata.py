...
SEARCH_KEYWORDS = [" ", "통계", "지방", "지표", "데이터", "자료", "인구", "건강", "노령"]
BASE_URL = "https://kosis.kr/openapi/statisticsSearch.do"
RESULTS_PER_PAGE = 1000 # API가 지원하는 최대치 또는 적절한 값으로 설정
...
def extract_search_keywords():
    ...
    result = set()
    for name in names:
        result.update(name.strip().split(" "))
    return result
...
def collect_kosis_metadata():
    ...
    for keyword in SEARCH_KEYWORDS:
        ...
        while True:
            ...
            try:
                print(f"  - 페이지 {page_num} 요청 중... (현재까지 수집된 고유 통계: {len(final_results)}개)")
                
                response = requests.get(BASE_URL, params=params, timeout=30)
                corrected_text = re.sub(r'([{,])\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*:', r'\1"\2":', response.text)
                ...
                print(f"  - 페이지 {page_num} 처리 완료. {new_items_count}개의 새로운 통계를 추가했습니다.")
                ...
                print("  [오류] JSON 파싱 실패. 응답이 올바른 JSON 형식이 아닙니다. 다음 검색어로 넘어갑니다.")
                break
            except Exception as e:
